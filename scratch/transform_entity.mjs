...
// 1. Imports
if (!content.includes("import { AIState }")) {
    content = content.replace(
...
for (const prop of propsToRemove) {
    ...
    const regex = new RegExp(`^[ \\t]*${escapedProp}\\r?\\n`, 'gm');
...
if (!content.includes('get behaviorState()')) {
    content = content.replace(
...
for (const p of propsToRemoveJson) {
    content = content.replace(p, '');
}

// 6. fromJSON
const propsToRemoveFromJson = [
...
for (const p of propsToRemoveFromJson) {
    content = content.replace(p, '');
}

fs.writeFileSync(filepath, content, 'utf8');
console.log("Done transforming Entity.js");
