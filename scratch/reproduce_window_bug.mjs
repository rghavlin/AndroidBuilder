...
async function testWindowBug() {
    console.log("--- Testing Swapped Fallbacks + Direct Step Bypass Zombie Window Breaching ---");
    ...
    // Setup zombie's target memory, but force sight check to be false
    zombie.currentTarget = { id: player.id, type: 'player' };
    zombie.setTargetSighted(9, 10); // LKP is (9, 10)
    zombie.canSeeEntity = () => false;
...
testWindowBug().catch(console.error);
