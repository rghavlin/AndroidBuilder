...
## Corpus Check
- 543 files · ~5,934,320 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3426 nodes · 8563 edges · 176 communities (126 shown, 50 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 131 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e7aedb03`
...
Cohesion: 0.11
Nodes (19): AudioSystem, FireSystem, testCornerBug(), testDiagonalBug(), build(), run(), runOscillationTest(), runTest() (+11 more)

### Community 1 - "UI Components"
Cohesion: 0.15
Nodes (16): BuildingTypes, isSpecialBuilding(), BY_SIZE, EDGE_CW, FLOORPLAN_FOOTPRINTS, FLOORPLANS, FRONTAGE_TURNS, orientFloorplan() (+8 more)
...
Cohesion: 0.07
Nodes (26): NOTE: equipItem intentionally has NO "Items inside" guard (unlike moveItem)., _warnedCatchAllProps, ItemDefs, CategoryDisplayName, CategoryPriority, EquipmentSlot, FireMode, FUEL_VALUES (+18 more)
...
Cohesion: 0.05
Nodes (19): AIState, Rabbit, SequencerAction, gm, serialized, aiComp, ent, json (+11 more)
...
Cohesion: 0.06
Nodes (18): MAP_GEN_CONFIG, doorsForBuilding(), PLAYER_FLOOD_OPTS, validateConnectivity(), isInsideCompound(), NOTE: previously spawned a 'placeable.help' ("?") item on every legacy, TemplateMapGenerator, isInsideBuilding() (+10 more)
...
Cohesion: 0.03
Nodes (74): JournalUI(), TradeDialog(), TradeDialogProps, AttachmentSlot, AttachmentSlotProps, FloatingContainerOverlayProps, WeaponModPanel(), WeaponModPanelProps (+66 more)
...
Cohesion: 0.05
Nodes (32): escalateFactionAgainstPlayer(), factionOf(), getAttackableTurretOnTile(), getCarriedPoweredTurret(), getExposedTurretTargets(), getPoweredTurretOnTile(), isPoweredTurret(), NOTE: wagon-nested turrets are not surfaced here yet (they'd need their tile (+24 more)
...
Cohesion: 0.06
Nodes (44): AITargeting, TurretAI, DestroyIntent, NoiseEvent, getSightRangeForHour(), engine, SimulationManager, NOTE: Structural damage (hp reduction, break/open flags) was already (+36 more)
...
Cohesion: 0.16
Nodes (9): PlaceIcon, findSouthTransitionTile(), isInsideTollGate(), isInStartArea(), logger, ZombieReplenishmentSystem, NOTE: this only PLACES the gate. The turret-firing rules during a toll run, ZombieSpawner (+1 more)
...
Cohesion: 0.05
Nodes (36): BlueprintRegistry, Inventory, EntityFactory, testHuntingDoorBug(), testWindowOscillations(), testWindowBug(), testZombieBug(), test911() (+28 more)
...
Cohesion: 0.15
Nodes (3): BaseMapGenerator, LAYOUT, ScenarioMapGenerator
...
Cohesion: 0.11
Nodes (33): BarterWindow(), BarterWindowProps, EarbucksShopWindow(), EarbucksShopWindowProps, ShopItemRowProps, InventoryPanel(), TollWindow(), TollWindowProps (+25 more)

### Community 65 - "Scenario Map Generation"
Cohesion: 0.06
Nodes (49): ActionSlotButton(), ActionSlotButtonProps, EarbucksDisplay(), ShopItemRow(), GameEventLog(), GameEventLogProps, getLogColor(), getLogColor() (+41 more)

### Community 66 - "Form UI Components"
Cohesion: 0.13
Nodes (31): DefeatDialog(), GameControls(), GameControlsProps, STAT_COLORS, StatBar, StatBarProps, GameScreenContent(), InfectionHUD() (+23 more)
...
Cohesion: 0.13
Nodes (10): DamageIntent, getZombieType(), IntentQueue, AISystem, spitAtPlayer(), CombatSystem, MovementSystem, VisionSystem (+2 more)
...
Cohesion: 0.19
Nodes (6): getProgressionForMap(), LootProgression, MapProgression, AnimalSpawner, NPCSpawner, runVerification()
...
Cohesion: 0.12
Nodes (23): clearOfOccupied(), contactSides(), footprintPlaceable(), FURNISH_PLAN, FURNITURE_FOOTPRINTS, furniturePieceOnFloor(), HEAD_SIDE, planFurniture() (+15 more)
...
Cohesion: 0.14
Nodes (17): AttributeCard(), AttributeCardProps, CompactSkillRow(), CompactSkillRowProps, SkillProgressBar(), SkillProgressBarProps, AttributeProgressionManager, applySurvivalCascade() (+9 more)
...
Cohesion: 0.13
Nodes (15): InventoryExtensionWindowProps, PlayerSkillsWindowProps, FloatingContainer(), FloatingContainerProps, FloatingContainerOverlay(), GridSizeContext, GridSizeContextType, GridSizeProvider() (+7 more)
...
Cohesion: 0.06
Nodes (35): DevConsole(), ActionContext, ActionProvider(), AudioProvider(), CameraProvider(), CombatContext, CombatProvider(), resolveTileTarget() (+27 more)
...
Cohesion: 0.08
Nodes (15): MoveIntent, EntityType, NPCTypes, NOTE: do NOT force itemsModified for every container/attachment item., Pathfinding, runTest(), testResults, door (+7 more)
...
## Knowledge Gaps
- **878 isolated node(s):** `inputContent`, `ActionSlotButtonProps`, `BarterWindowProps`, `BuildingTooltipProps`, `CharacterCreatorProps` (+873 more)
...
- **Why does `cn()` connect `Action Queue Processing` to `Entity Spawning and Scent`, `Character and Menu Windows`, `World Progression and Spawning`, `table.tsx`, `navigation-menu.tsx`, `Sidebar UI Components`, `Blueprint and Inventory Registry`, `Menubar UI Components`, `Ground Item Management`, `Scenario Map Generation`, `Form UI Components`, `context-menu.tsx`, `Table UI Components`, `Music and Playlist Manager`, `Zombie Line-of-Sight Testing`, `Book Stats Initialization`, `SurvivalCascade.js`, `.getBeltContainers`, `OTP Input Components`, `TurretCombat.js`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `createItemFromDef()` connect `Shop and Log UI` to `Item Components`, `Item Interaction Logic`, `AI and Inventory Systems`, `verify_army_tent.js`, `Tooltip Components`, `Entity Spawning and Scent`, `Map Template Generation`, `Inventory Management System`, `Map Generation Config`, `EventRunner`, `Options and Crafting UI`, `Road and Town Generation`, `Map Editor Tools`, `Crafting Manager Logic`, `ImageLoader`, `Pathfinding.js`, `pagination.tsx`, `Entity Serialization Tests`, `Crafting Recipe Verification`, `Ground Item Management`, `Scenario Map Generation`, `Form UI Components`, `Carousel UI Components`, `World Object Spawning`, `.executeTransition`, `verify_molotov.mjs`, `FurniturePlanner.js`, `Crop Growth Verification`, `DevConsole.tsx`, `Building Hallway Tests`, `Seeded Random Utilities`, `Consumable`?**
