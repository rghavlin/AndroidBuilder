...
- **God-objects are under managed decomposition.** `entities/Entity.js`, `map/GameMap.js`, `inventory/InventoryManager.js`, `inventory/Item.js`, `map/LootGenerator.js`, `managers/SimulationManager.js`, and `pages/editor.tsx` each carry more than one responsibility and are being split opportunistically. **Do not add new responsibilities to them.** New behavior belongs in a system/module operating on components — for `editor.tsx`, in a panel component under `components/MapEditor/`. When you touch one of these files, extract the cleanest seam rather than growing it. Each carries a header banner pointing here. Extraction targets are listed in `CODE_QUALITY_ACTION_PLAN.md` (Wave 4).
- **The no-growth half of that rule is enforced mechanically.** `test/quality/godObjectBudget.test.js` runs in `npm test` and fails when any god-object exceeds its recorded line budget in `scripts/god-object-budget.json`. The budget only ever ratchets **down**: after you extract a seam, run `npm run budget:update` to lock in the smaller size. `budget:update` refuses to raise a budget — raising one requires hand-editing the JSON, so growth always shows up as a deliberate line in the diff rather than as silent drift.
