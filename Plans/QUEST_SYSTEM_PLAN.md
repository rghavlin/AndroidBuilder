...
- **Phase 5 — DONE.** Map Entity Registry & NPC/Player Movement.
  ...
  - **"Attack entity" event step:** An `attackEntity` step, added alongside `moveEntity` (2026-08-24). Names an attacker (registered NPC or zombie) and a victim (the player or another registered entity), plus a weapon mode — `auto` shoots when the attacker carries a ranged weapon, `melee`/`ranged` force the choice. Being scripted, it always hits and can never be dodged (`CombatResolver`'s `alwaysHit`, for authored attacks only — damage and crits still roll), and it ignores AP, range and line of sight. Because no simulation pass follows an event it resolves the whole beat itself: roll (`systems/ScriptedAttack.js`), animation and PLAYBACK-FIRST damage (`TurnManager.playScriptedAction`), then the corpse if the blow kills. Covered by `test/quest/attackEntityStep.test.js`.
  - **"Infect / cure player" event step:** A `setInfection` step (2026-08-24) that hands the player the zombie virus — the lethal viral clock ticked by `SurvivalCascade.tickInfection`, not the recoverable Disease counter or a rag-bound wound infection. Infect or cure, with an optional authored countdown in hours (default 24); an explicit countdown re-times an existing infection, while a plain re-infect never resets a clock already running. Cure clears the flag and cancels any brain-pulp treatment — nothing else in the game can do that, since pulp only ever pauses the virus. Engine side lives in `utils/SurvivalCascade.js` (`infectPlayer`/`cureInfection`), keeping the whole infection lifecycle in one module. Covered by `test/quest/setInfectionStep.test.js`.
