  ...
  static getMovementCost(x1, y1, x2, y2, targetTile = null, options = {}) {
    ...
    if (targetTile && targetTile.contents && Array.isArray(targetTile.contents)) {
      ...
      if (window) {
        if (options?.isZombie) {
          if (window.isBroken) {
            // Phase 18 Final Fix: Broken windows are now standard movement cost (1.0)
            baseCost = 1.0;
          } else {
            // Phase 18 Final Fix: Closed windows are now only slightly penalized (2.0)
            // to ensure zombies commit to breaking in rather than detouring.
            baseCost = 2.0; 
          }
        } else {
          baseCost += 1;
      ...
      if (options?.isZombie) {
        ...
        if (door && !door.isOpen) {
          // Phase 18 Final Fix: Closed doors match closed windows (2.0)
          baseCost = 2.0;
        }
