...
export default function StartModeDialog({ onSelect }: StartModeDialogProps) {
  return (
    ...
    >
      <div className="bg-card border border-border p-6 rounded-2xl max-w-xs w-full shadow-[0_0_50px_rgba(0,0,0,0.85)] relative flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            className="flex items-center gap-4 p-4 rounded-xl border border-border bg-secondary hover:bg-secondary/80 hover:border-border transition-all duration-200 group text-left cursor-pointer w-full"
            data-testid="normal-start-option"
          >
            <div className="p-2.5 bg-muted rounded-lg border border-border group-hover:scale-105 transition-transform duration-200 flex items-center justify-center">
              <Flame className="h-5 w-5 text-muted-foreground" />
            </div>
            <h3 className="text-base font-black text-foreground uppercase tracking-wider select-none">
              Normal Start
            </h3>
          </button>
        </div>
