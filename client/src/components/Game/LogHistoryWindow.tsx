...
export default function LogHistoryWindow({ onClose }: LogHistoryWindowProps) {
    ...
    return (
        <div className={cn(
            ...
            isLightTheme(theme) ? "bg-background shadow-lg" : "bg-background/50"
        )}>
            {/* Header */}
            ...
            <div 
                ...
                className={cn(
                    ...
                    isLightTheme(theme) ? "bg-zinc-50" : "bg-black/20"
                )}
            >
                {logs.length === 0 ? (
                        ...
                        {logs.map((log) => (
                            <div 
                                ...
                                className={cn(
                                    ...
                                    isLightTheme(theme) ? "border-zinc-200/60" : "border-white/5"
                                )}
                            >
                                <div className="flex items-start gap-3">
                                    <span className={cn("text-[10px] font-mono pt-0.5 shrink-0", isLightTheme(theme) ? "text-zinc-400" : "text-zinc-500")}>
                                        [{log.timestamp}]
                                    </span>
                                    <div 
                                    ...
                                    >
                                        <span className={cn("text-[10px] uppercase font-black mr-2", isLightTheme(theme) ? "opacity-90" : "opacity-60")}>
                                            {log.type}
                                        </span>
